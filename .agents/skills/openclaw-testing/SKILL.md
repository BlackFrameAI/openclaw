...
explicit `--provider blacksmith-testbox` unless the user requests another
backend or the task requires one. Untrusted contributor or fork code must use
...
1. Inspect the diff and classify the touched surface:
   - any trusted agent-run test, focused or broad: run it on the pre-warmed
     Blacksmith Testbox backend; use another backend only when the task requires
